        ...
        <div className="grid gap-12 relative">
          ...
          {ACHIEVEMENTS.map((achievement, index) => (
            <motion.div
              ...
              className="grid md:grid-cols-[60px_1fr] gap-8"
            >
              {/* Year Bubble */}
              ...
              {/* Content Card */}
              <div className="bg-surface border border-white/10 rounded-3xl p-8 hover:border-white/20 transition-all group">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                  <div>
                    <div className="md:hidden inline-block px-3 py-1 bg-white/5 rounded-full text-xs font-bold text-gray-300 mb-3 border border-white/10">
                      {achievement.year}
                    </div>
                    <h3 className="text-2xl font-bold text-white">{achievement.title}</h3>
                  </div>
                  <a href={achievement.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm font-medium text-gray-400 group-hover:text-white transition-colors">
                    Baca Refleksi <ArrowRight className="w-4 h-4" />
                  </a>
                </div>
